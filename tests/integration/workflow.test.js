...
describe('Integration: API Workflow', () => {
  ...
  describe('API Jobs Core', () => {
    ...
    itIfApi('should query jobs by CIF and return valid data', async () => {
      ...
      if (result.numFound === 0) {
        console.log('⚠️ No TAKTILE jobs in API — skipping job field assertions (scraper may not have run yet)');
        return;
      }
  ...
  describe('Full Validation Workflow', () => {
    ...
    itIfAnaf('should complete the ANAF → Peviitor validation path', async () => {
      ...
      const taktileCompany = searchResults.find(c =>
        c.cui.toString() === COMPANY_CIF && c.statusLabel === 'Funcțiune'
      );
      expect(taktileCompany).toBeDefined();

      const anafData = await anaf.getCompanyFromANAF(taktileCompany.cui.toString());
    ...
    itIfApi('should validate company and query API for existing jobs', async () => {
      ...
      if (companyResult.existingJobsCount === 0) {
        console.log('⚠️ No TAKTILE jobs in API — skipping job count assertion (scraper may not have run yet)');
        return;
      }
