...
const baseCompany = {
  ...
  career: ["https://jobs.ashbyhq.com/taktile"],
  lastScraped: "2026-06-17"
};

const baseJob = {
  url: "https://jobs.ashbyhq.com/taktile/en/vacancy/123_en",
...
describe("generateJobsMarkdown", () => {
  describe("company section", () => {
    it("includes company name as h1", () => {
      ...
      expect(md).toContain("# TAKTILE S.R.L.");
    });

    it("includes CIF", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("51981214");
    });

    it("includes brand", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("TAKTILE");
    });
    ...
    it("includes website as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://www.taktile.com](https://www.taktile.com)");
    });

    it("includes career page as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://jobs.ashbyhq.com/taktile](https://jobs.ashbyhq.com/taktile)");
    });
    ...
    it("omits optional fields when not present", () => {
      const minimal = { id: "51981214", company: "TAKTILE S.R.L." };
  ...
  describe("jobs section", () => {
    ...
    it("includes job URL as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, [baseJob]);
      expect(md).toContain("[https://jobs.ashbyhq.com/taktile/en/vacancy/123_en]");
    });
    ...
    it("renders multiple jobs", () => {
      const job2 = { ...baseJob, title: "DevOps Engineer", url: "https://jobs.ashbyhq.com/taktile/en/vacancy/456_en" };
    ...
    it("handles job with no optional fields", () => {
      const minimal = { url: "https://jobs.ashbyhq.com/taktile/en/vacancy/999_en", title: "QA Engineer" };
