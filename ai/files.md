...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js`, `tests/validate-taktile-jobs.js`, and the deep-validate workflow. |
...
| `tests/validate-taktile-jobs.js` | **Company-specific validator (used by CI).** Modes: `--head` (default), `--content`, `--browser` (Playwright). Called nightly by `automation-testing.yml` and manually via `job-deep-validate.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real Taktile Ashby API, ANAF, and Peviitor API |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for the Taktile Ashby board |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape Taktile (Ashby) → transform → upsert → generate docs/jobs.md
