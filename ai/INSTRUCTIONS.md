...
This scraper extracts job listings from the Taktile Ashby job board and imports them to peviitor.ro.

Target: https://jobs.ashbyhq.com/taktile
...
1. **Start with brand** - We know the brand ("TAKTILE")
...
8. **Scrape new jobs** - Extract jobs from the Taktile Ashby job board (filter to Romania/Iasi)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Taktile Ashby jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from the Taktile Ashby job board (filter to Romania/Iasi only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Taktile Ashby API (jobs filtered to Romania/Iasi)
    │
    ▼
...
- **Taktile Ashby API**: `https://api.ashbyhq.com/posting-api/job-board/taktile` — GET, returns `{ jobs: [...] }` with `id`, `title`, `location`, `secondaryLocations[]`, `workplaceType` (Remote/Onsite/Hybrid), `jobUrl`
...
| Concurrency | 1 (sequential) | Single fetch of the Ashby board — no pagination |
| User-Agent | `job_seeker_ro_spider` | Identifies the scraper in server logs |
| Request delay | 1000 ms between operations | `scraper/index.js` — `sleep(1000)` |
